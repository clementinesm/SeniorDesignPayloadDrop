...
def run_mission(target_coord):
    ...
    # write waypoints to the pixhawk using MAVROS  
    # - insert function call -


    
    # read the waypoints and send to mission planner!
    #wp = mavwp.MAVWPLoader() # was an old pymavlink command
    ...
    with open(mission_file_name) as f:
        for i, line in enumerate(f):
            if i==0:
                ...
                # each waypoint is a new line!
                wp = Waypoint() # from mavros_msg.msg
                ...
                #ln_seq = int(linearray[0])
                wp.is_current = int(linearray[1])
                wp.frame = int(linearray[2])
                wp.command = int(linearray[3])
                wp.param1=float(linearray[4])
                wp.param2=float(linearray[5])
                wp.param3=float(linearray[6])
                wp.param4=float(linearray[7])
                wp.x_lat=(float(linearray[8]))
                wp.y_long=(float(linearray[9]))
                wp.z_alt=float(linearray[10])
                # might be a bool, check this
                wp.autocontinue = int(float(linearray[11].strip()))
